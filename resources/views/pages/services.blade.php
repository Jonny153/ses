@extends('layout')

@section('title', 'О компании')

@section('content')
<div class="content">

    <section id="services" class="container">

        <nav aria-label="breadcrumb">
            <ol class="breadcrumb bg-transparent">
                <li class="breadcrumb-item"><a href="/">Главная</a></li>
                <li class="breadcrumb-item active" aria-current="page">Услуги</li>
            </ol>
        </nav>

        <div class="d-flex flex-column py-5">
            <h1 class="header align-self-center" data-aos="flip-left" >Услуги</h1>
        </div>

        <div class="row">
            <div class="col-12 p-4 mh-100">
                <div class="row service-block h-100 d-flex">
                    <div class="col-12 col-lg-3 bg-light pl-3 pr-0">
                        <a href="/services/dezinsection">
                            <picture class="float-right float-lg-none">
                                <img srcset="/img/content/dezinsect.jpg" alt="responsive image" data-aos="fade-up" class="d-block img-fluid">
                            </picture></a>
                    </div>
                    <div class="col-12 col-lg-9 p-3" data-aos="fade-left">
                        <h3 class="mb-4"><a class="black" href="/services/dezinsection">Дезинсекция</a></h3>
                        <p class="lead">Уничтожение насекомых</p>
                        <p>тараканов, клопов, блох,
                            клещей, гусениц, тли,
                            пауков, мокриц, мух,
                            комаров, шерней, ос,
                            муравьев, всех бытовых
                            насекомых и вредителей
                            растительности</p>
                    </div>
                    <div class="col-12 bg-light mt-2 align-self-end" data-aos="flip-up" data-aos-delay="300">
                        <a class="btn btn-primary btn-pulse  float-lg-right m-3" href="/services/dezinsection">Подробнее</a>
                    </div>
                </div>
            </div>

            <div class="col-12 p-4 mh-100">
                <div class="row service-block h-100 d-flex">
                    <div class="col-12 col-lg-3 bg-light pl-3 pr-0">
                        <a href="/services/deratization">
                            <picture class="">
                                <img srcset="/img/content/derat.jpg" alt="responsive image" data-aos="fade-up" class="d-block img-fluid">
                            </picture></a>
                    </div>
                    <div class="col-12 col-lg-9 p-3" data-aos="fade-left">
                        <h3 class="mb-4"><a class="black" href="/services/deratization">Дератизация</a></h3>
                        <p class="lead">Уничтожение грызунов</p>
                        <p>мышей, крыс, кротов,
                            сусликов, полевок</p>
                    </div>
                    <div class="col-12 bg-light mt-2 align-self-end" data-aos="flip-up" data-aos-delay="300">
                        <a class="btn btn-primary btn-pulse  float-lg-right m-3" href="/services/deratization">Подробнее</a>
                    </div>
                </div>
            </div>

            <div class="col-12 p-4 mh-100">
                <div class="row service-block h-100 d-flex">
                    <div class="col-12 col-lg-3 bg-light pl-3 pr-0">
                        <a href="/services/dezinfection">
                            <picture class="float-right float-lg-none">
                                <img srcset="/img/content/dezinfect.jpg" alt="responsive image" data-aos="fade-up" class="d-block img-fluid">
                            </picture></a>
                    </div>
                    <div class="col-12 col-lg-9 p-3" data-aos="fade-left">
                        <h3 class="mb-4"><a class="black" href="/services/dezinfection">Дезинфекция</a></h3>
                        <p class="lead">Уничтожение микробов</p>
                        <p>уничтожение грибка,
                            плесени, вирусов,
                            инфекции</p>
                    </div>
                    <div class="col-12 bg-light mt-2 align-self-end" data-aos="flip-up" data-aos-delay="300">
                        <a class="btn btn-primary btn-pulse  float-lg-right m-3" href="/services/dezinfection">Подробнее</a>
                    </div>
                </div>
            </div>

            <div class="col-12 p-4 mh-100">
                <div class="row service-block h-100 d-flex">
                    <div class="col-12 col-lg-3 bg-light pl-3 pr-0">
                        <a href="/services/dezodoration">
                            <picture class="">
                                <img srcset="/img/content/dezodor.jpg" alt="responsive image" data-aos="fade-up" class="d-block img-fluid">
                            </picture></a>
                    </div>
                    <div class="col-12 col-lg-9 p-3" data-aos="fade-left">
                        <h3 class="mb-4"><a class="black" href="/services/dezodoration">Дезодорация</a></h3>
                        <p class="lead">Устранение  различных
                            неприятных запахов</p>
                        <p>сырости, гнили, мочи
                            животных, трупный запах,
                            въевшиеся запахи табака.
                            В квартирах, автомобилях,
                            подвальных, и подсобных
                            помещениях</p>
                    </div>
                    <div class="col-12 bg-light mt-2 align-self-end" data-aos="flip-up" data-aos-delay="300">
                        <a class="btn btn-primary btn-pulse  float-lg-right m-3" href="/services/dezodoration">Подробнее</a>
                    </div>
                </div>
            </div>
        </div>

    </section>
</div>

@endsection

@section('scripts')

@endsection